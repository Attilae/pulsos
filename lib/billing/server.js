...
import { billingSubscriptions, entitlementUsage } from '@/lib/db/schema.js'
import {
  FREE_LIMITS, PRO_LIMITS, isSubscriptionEntitled, metricLimit, usagePeriod,
} from './plans.js'
...
export async function getEntitlements(userId, now = new Date()) {
  ...
  const plan = isSubscriptionEntitled(subscription, now) ? 'pro' : 'free'
...
export async function claimUsage(userId, metric, now = new Date()) {
  ...
  const limit = metricLimit(entitlements.plan, metric)

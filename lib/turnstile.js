...
// A site reachable on both the apex and `www.` mints tokens on whichever one
// the visitor is actually on, so allowing one and not the other rejects real
// submissions — which is what happened in production ("hostname not allowed:
// www.layth.space") while the app's configured URL was the apex. Pairing them
// widens nothing meaningful: both names sit under the same registrable domain,
// which only the operator can point anywhere.
function withWwwVariants(names) {
  for (const name of [...names]) {
    if (name.startsWith('www.')) names.add(name.slice(4))
    // Skip bare IPs — `www.127.0.0.1` is not a thing.
    else if (name.includes('.') && !/^[\d.]+$/.test(name)) names.add(`www.${name}`)
  }
  return names
}
...
export function allowedHostnames() {
  ...
  if (configured.length) return withWwwVariants(new Set(configured))

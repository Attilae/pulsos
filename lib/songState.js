...
export function applySnapshot(snapshot, setters, engine, routes) {
  ...
  setters.setDisabledRoutes?.(s.muted ?? {})
  ...
  setters.setTrackEqs?.(migrateTrackEqs(s.trackEqs))
  ...
  // Per-stop pitch offsets: prefer the general map; migrate legacy per-duplicate
  // storage when a pre-unification snapshot has no trackPitchOffsets.
  setters.setTrackPitchOffsets?.(s.trackPitchOffsets ?? _hoistLegacyPitchOffsets(s))
  ...
  // Duplicate lanes: restore descriptors and reconstruct their clone routes so the
  // engine replay below can resolve their (synthetic) ids to a real stop list.
  const duplicates = s.duplicates ?? []
  setters.setDuplicates?.(duplicates)

  // Merged lanes: restore descriptors and reconstruct their synthetic PolySynth
  // route (carrying source geometry) so the engine replay can resolve their ids.
  const merges = s.merges ?? []
  setters.setMerges?.(merges)

  // Drum backing (Map/DAW): restore React state; engine replay happens below.
  setters.setDrumPattern?.(s.drumPattern ?? null)
  const dupRoutes = duplicates
    .map(d => {
      const src = (routes ?? []).find(r => r.id === d.sourceId)
      return src ? { ...src, id: d.id, name: d.name, sourceId: d.sourceId, isDuplicate: true } : null
    })
    .filter(Boolean)
  const baseById = new Map((routes ?? []).map(r => [r.id, r]))
  const mergeRoutes = merges
    .map(m => {
      const srcRoutes = (m.sourceIds ?? []).map(id => baseById.get(id)).filter(Boolean)
      if (!srcRoutes.length) return null
      const base = srcRoutes[0]
      return { ...base, id: m.id, name: m.name, type: base.type, isMerged: true, sourceIds: m.sourceIds, sourceRoutes: srcRoutes }
    })
    .filter(Boolean)
  ...
  if (engine) _applyToEngine({ ...s, automationCfg }, engine, [...(routes ?? []), ...dupRoutes, ...mergeRoutes])
}
...
function _applyToEngine(s, engine, routes) {
  ...
  // Prefer the general map; migrate legacy per-duplicate storage for old songs.
  const pitchOffsets = s.trackPitchOffsets ?? _hoistLegacyPitchOffsets(s)
  for (const [rid, map] of Object.entries(pitchOffsets)) {

...
export function buildSnapshot(s) {
  return {
    ...
    // Duplicate lanes: descriptors only ({ id, sourceId, name, perStopSteps }) —
    // the clone routes are reconstructed from the base routes on load.
...
function _applyToEngine(s, engine, routes) {
  ...
  // Duplicate-lane per-stop diatonic pitch offsets (applied when each Part builds)
  for (const d of s.duplicates ?? []) {
    if (d?.perStopSteps && Object.keys(d.perStopSteps).length) {
      try { engine.setPitchOffsets?.(d.id, d.perStopSteps) } catch (e) { console.warn('apply setPitchOffsets', e) }
    }
  }

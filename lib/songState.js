...
import { normalizeIrType } from './fxSpecs.js'
import {
  resolveSnapshotLanes, snapshotBaseRouteIds, snapshotLaneDisabledMap,
} from './songLanes.js'
import { normalizeLaneTags } from './laneTags.js'
import { normalizeNoteChanceMap, normalizeStopChanceMap, normalizeLoopPatternMap } from './laneGating.js'
import { normalizeNoteLength, normalizeNoteLengthMap } from './noteLength.js'
...
// The pitch contour every song written before schemaVersion 4 played by default.
// `trackPitchVariety` is sparse — only lanes the user (or an AI plan) explicitly
// configured have an entry — so flipping DEFAULT_PITCH_VARIETY to 'demand' would
// re-pitch every untouched lane of every old song. Pin those lanes instead.
const LEGACY_PITCH_CONTOUR = 'geographic'

function _pinLegacyPitchContour(s) {
  const out = { ...(s.trackPitchVariety ?? {}) }
  const ids = new Set([
    ...snapshotBaseRouteIds(s),
  ...
  for (const id of ids) {
    if (!id) continue
    const cur = out[id]
    out[id] = { variety: 0, ...(cur ?? {}), contour: cur?.contour ?? LEGACY_PITCH_CONTOUR }
  }
  return out
}
...
export function buildSnapshot(s) {
  return {
    ...
    trackSidechains: s.trackSidechains ?? {},
    ...
    // Note chance + loop rest patterns (lib/laneGating.js). Sparse; absent in
    // older saves, where {} correctly means "every lane always plays".
    trackNoteChances:  normalizeNoteChanceMap(s.trackNoteChances),
    trackStopChances:  normalizeStopChanceMap(s.trackStopChances),
    trackLoopPatterns: normalizeLoopPatternMap(s.trackLoopPatterns),
    // Per-lane note length (lib/noteLength.js). Sparse; absent = the legacy gate.
    trackNoteLengths:  normalizeNoteLengthMap(s.trackNoteLengths),
    ...
    // Per-lane role labels: routeId → { text, color }. Annotation only; absent in
    // pre-label saves, where {} is the correct meaning ("no lane is labelled").
    trackLabels:     normalizeLaneTags(s.trackLabels),
...
export function migrateSnapshot(raw, version) {
  ...
  if (from < 4) {
    // The default pitch contour became 'demand'. Old songs never stored a contour
    // for lanes they didn't tweak, so record the one they actually played.
    s.trackPitchVariety = _pinLegacyPitchContour(s)
  }
  ...
  // Removed reverb IRs load as their closest replacement, so the IR picker shows
  // a real option instead of an id it no longer lists.
  const irType = s.fxBusParams?.reverb?.irType
  if (irType && normalizeIrType(irType) !== irType) {
    s.fxBusParams = { ...s.fxBusParams, reverb: { ...s.fxBusParams.reverb, irType: normalizeIrType(irType) } }
  }
  return s
}
...
export function applySnapshot(snapshot, setters, engine, routes) {
  ...
  setters.setTrackSidechains?.(s.trackSidechains ?? {})
  ...
  setters.setTrackNoteChances?.(normalizeNoteChanceMap(s.trackNoteChances))
  setters.setTrackStopChances?.(normalizeStopChanceMap(s.trackStopChances))
  setters.setTrackLoopPatterns?.(normalizeLoopPatternMap(s.trackLoopPatterns))
  setters.setTrackNoteLengths?.(normalizeNoteLengthMap(s.trackNoteLengths))
  // Re-normalized on the way in too: a snapshot can arrive from a shared link,
  // and the colour ends up in an inline style.
  setters.setTrackLabels?.(normalizeLaneTags(s.trackLabels))
...
function _applyToEngine(s, engine, routes, laneIds) {
  ...
  const routeIds = new Set([
    ...
    ...Object.keys(s.trackNoteChances ?? {}),
    ...Object.keys(s.trackStopChances ?? {}),
    ...Object.keys(s.trackLoopPatterns ?? {}),
    ...Object.keys(s.trackNoteLengths ?? {}),
  ])
  ...
  for (const rid of routeIds) {
    ...
    // Order-independent: a cfg naming a source route that isn't installed yet just
    // sits in the engine's index and starts ducking once that route fires.
    const sidechain = s.trackSidechains?.[rid]
    if (sidechain) { try { engine.setSidechain?.(rid, sidechain) } catch (e) { console.warn('apply setSidechain', e) } }
    ...
    // Always pushed (null clears): the engine may be reused across songs.
    const noteChances  = normalizeNoteChanceMap(s.trackNoteChances)
    const stopChances  = normalizeStopChanceMap(s.trackStopChances)
    const loopPatterns = normalizeLoopPatternMap(s.trackLoopPatterns)
    try {
      engine.setNoteChance?.(rid, noteChances[rid] ?? null)
      engine.setStopChances?.(rid, stopChances[rid] ?? {})
      engine.setLoopPattern?.(rid, loopPatterns[rid] ?? null)
      engine.setNoteLength?.(rid, normalizeNoteLength(s.trackNoteLengths?.[rid]))
    } catch (e) { console.warn('apply lane gating', e) }

...
// Read + strip a one-shot query param (so a refresh doesn't act on it again).
function _consumeParam(name) {
...
// ?shared=<id> imports a public share link as a detached copy.
const _consumeSharedParam = () => _consumeParam('shared')
// ?song=<id> opens one of the signed-in user's own songs — the link the MCP
// tools hand back after saving one. Only read once signed in, so a signed-out
// visitor keeps it in the URL until they sign in and the hydration pass re-runs.
const _consumeSongParam = () => _consumeParam('song')
...
export function useSongPersistence({ state, applyPreset, routes, onReset }) {
  ...
  // ── Hydration ──────────────────────────────────────────────────────────────
  // Priority: a ?shared=<id> link (public, no sign-in needed) → otherwise a
  // ?song=<id> link to one of the user's own songs → their lastSongId.
  useEffect(() => {
    ...
    ;(async () => {
      ...
      hydratedRef.current = true
      await refreshSongs()
      const linkedId = _consumeSongParam()
      if (linkedId) setLastSongId(linkedId)
      const lastId = linkedId ?? getLastSongId()
      if (lastId) {

...
import { normalizeLoopPattern, patternPeriod } from './laneGating.js'
...
function laneDescriptor(id, route, s) {
  ...
  const rawPattern = s.trackLoopPatterns?.[id]
  const pattern    = rawPattern && normalizeLoopPattern(rawPattern).rest > 0 ? normalizeLoopPattern(rawPattern) : null
  return {
    id,
    ...
    // Loop rest pattern (lib/laneGating.js), or null when the lane plays every loop.
    pattern,
    periodUnits: loopUnits * patternPeriod(pattern),
  }
}
...
export function describeSnapshotLoops(snapshot, routes, { activeLaneLimit = null } = {}) {
  ...
  if (drumsAudible) {
    out.push({
      ...
      pattern: null,
      periodUnits: DRUM_LOOP_UNITS,
    })
  }

  // A lane with a loop rest pattern only repeats itself every play+rest loops,
  // so the realign point is taken over that whole period, not one loop.
  const cycle = cycleFromUnits(out.map(l => l.periodUnits))
  return {
    lanes: out,

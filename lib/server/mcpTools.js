...
import { planAdvisories } from '../ai/planAdvisories.js'
...
export function createLeidMcpServer(userId, services = {}) {
  ...
  // planAdvisories then lists what it kept but will not sound as intended.
  ...
  const compose = async ({ cityId, base, plan, entitlement, mode }) => {
    ...
    return {
      ...
      report: {
        ...
        dropped,
        advisories: planAdvisories(validated, { mode, bpm: base?.bpm }),
  ...
  server.registerTool('get_composer_guide', {
    description: `Start here to compose music. Returns the musical and sound-design policy, the sound recipes, the full vocabulary (instruments, sampler presets, drum pads, FX buses and params, scales, ranges) and an example plan for writing a Leið loop plan in a city, plus your lane limit and the genre recipes available. Pass genre (one of ${RECIPE_IDS.join(', ')}) to include that recipe's tempo, roles, drum seed, effects, beat blueprint and matching sound recipes. Then pick lines with list_routes, check the plan with preview_song_plan, and save it with create_song_from_plan or apply_plan_to_song.`,
...
3. Design each role's sound from the guide's sound recipes (instrument, envelope fitted to the one-beat gate, tone, filter), then call preview_song_plan. Fix anything it reports as dropped and every advisory that matters, and preview again.

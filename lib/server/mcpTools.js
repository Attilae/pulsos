...
export function createLeidMcpServer(userId, services = {}) {
  ...
  const compose = async ({ cityId, base, plan, entitlement }) => {
    ...
    const applied = applyPlanToSnapshot(base, validated, { activeLaneLimit: limit })
    return {
  ...
  server.registerTool('get_composer_guide', {
    description: 'Start here to compose music. Returns the full vocabulary (instruments, sampler presets, drum pads, FX buses and params, scales, ranges) and an example plan for writing a Leið loop plan in a city, plus your lane limit. Then pick lines with list_routes, check the plan with preview_song_plan, and save it with create_song_from_plan or apply_plan_to_song.',
    inputSchema: z.object({ cityId: cityIdSchema }),
    annotations: { readOnlyHint: true },
  }, async ({ cityId }) => {
    ...
    return result({
      ...
      guide: buildComposerGuide({ cityName: city.name, maxTracks }),
    })
  })
  ...
  server.registerTool('preview_song_plan', {
    description: 'Check a loop plan without saving anything. Validates every route id, instrument, range and FX setting (see get_composer_guide), then reports what was dropped and summarizes the resulting song. Pass songId to preview an edit of an existing song instead of a new one.',
    inputSchema: z.object({
    ...
    const composed = await compose({ cityId, base, plan, entitlement })
  ...
  server.registerTool('create_song_from_plan', {
    description: 'Save a loop plan as a new song in your Leið library. The result includes openUrl, which opens the song in the Leið DAW; press Play there to hear it. This does not change a song that is already open in a browser tab.',
    inputSchema: z.object({
      cityId: cityIdSchema,
    ...
    if (!entitlement) return errorResult(PRO_REQUIRED)
    if (!cityById(cityId)) return errorResult(`Unknown city "${cityId}"; see list_cities`)
    const composed = await compose({ cityId, base: defaultSnapshot(cityId), plan, entitlement })
    if (composed.error) return errorResult(composed.error)
  ...
  server.registerTool('apply_plan_to_song', {
    ...
    const saved = await updateFor(userId, id, expectedUpdatedAt, async (song) => {
      ...
      const composed = await compose({ cityId: song.cityId, base: song.state, plan, entitlement })
...
1. Call get_composer_guide for "${cityId}" and follow its vocabulary and ranges exactly.
...
To change a saved song instead, read it with get_song, preview with its songId, and save with apply_plan_to_song using its updatedAt as expectedUpdatedAt; if that reports a conflict, read the song again rather than retrying.`,
      },
    }],
  }))

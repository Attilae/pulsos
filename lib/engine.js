...
export class TransitEngine {
  ...
  updateAutomationLane(routeId, laneId, cfg) {
    ...
    // Re-time the lane's mock part when its speed or source changes.
    if (cfg.speed !== undefined || cfg.sourceRouteId !== undefined) {
      this._rebuildAutomationLanePart(key)
    }
  }
  ...
  _buildAutomationLanePart(key) {
    ...
    const region      = this._trackLoopRegions[cfg.sourceRouteId]
    ...
    const gridStops = snapStopsToGrid(src.stops, src.totalDist)

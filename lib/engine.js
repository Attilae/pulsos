...
export class TransitEngine {
  ...
  _disposeRouteEntry(entry) {
    ...
    // Cleanly remove the weq8 EQ's output→panner edge before disposing the panner,
    // but keep the runtime alive (it persists across start/stop, owned by _eqRuntimes).
    try { entry.eq?.disconnect(entry.routePanner?.input) } catch {}
  ...
  _createSingleRouteEntry(routeId, routeType, synthType = 'Synth', envelope = null, scale = null) {
    ...
    eq.connect(routePanner.input)     // weq8 output → native StereoPannerNode
  ...
  _ensureDrumInsert() {
    ...
    const entry = { routeId: rid, routeType: null, synthType: null, routeGain, routePanner, filter, eq }

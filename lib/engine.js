...
// One sample is enough for the grain source — Tone.Sampler repitches from the
// nearest zone, so pick the preset note closest to the render note.
function granularRenderSampleUrl(synthType, params) {
  if (synthType === 'Drums') {
    const voice = DRUM_VOICES.find(v => v.id === params.drumVoice) ?? DRUM_VOICES[0]
    return DRUM_BASE_URL + voice.file
  }
  ...
  return preset.baseUrl + preset.urls[note]
}

function fetchGranularRenderSample(synthType, params) {
  return loadSample(granularRenderSampleUrl(synthType, params))
}

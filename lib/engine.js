...
import { unlockAudio } from './audioSession.js'
...
import { normalizeEqState, cloneSpec } from './eqMigrate.js'
import { resolveSamplerUrls, loadSample } from './sampleCache.js'
import { normalizeNoteLength } from './noteLength.js'
import { loopIndexAt, loopPlays, resolveNoteChance, rollChance, normalizeNoteChance, normalizeLoopPattern } from './laneGating.js'
import {
  geoToMidi, midiToNote, randomFromScale, shiftOctaveNote, shiftSemitones, denormalizeToRange, denormalizeExp,
  snapStopsToGrid, GRID_TOTAL_CELLS, GRID_BARS, GRID_STEPS_PER_BAR, GRID_RESOLUTION_STEPS_PER_BAR, DEFAULT_GRID_RESOLUTION,
  generatePitchMap, generateVelocityMap, DEFAULT_PITCH_VARIETY, routeBounds, SCALES, noteToMidi, MODES,
  buildArpSequence, buildChordArpSequence, transposeNoteInScale,
} from './mappings.js'
import {
  DRUMS_ROUTE_ID, SAMPLER_PRESETS, SAMPLER_PRESET_LIST, DRUM_BASE_URL,
  DRUM_TRIGGER_NOTE, DRUM_VOICES, DRUM_VOICE_LICENSE, DEFAULT_GRANULAR,
  DEFAULT_SIDECHAIN, SIDECHAIN_ANY_DRUM, SIDECHAIN_PAD_SOURCES, SYNTH_DEFAULTS,
  granularSourceZone,
} from './soundSpecs.js'
...
export { SYNTH_TYPES } from './soundSpecs.js'
...
// Pitch-variety option list + default (defined in mappings; re-exported for the UI).
export { PITCH_CONTOURS, DEFAULT_PITCH_VARIETY } from './mappings.js'
...
// The absolute sample URLs a sample-backed synth type needs, in Tone.Sampler
// zone-map shape. Empty for every synthesized voice. Exported so callers can warm
// them ahead of time (see prefetchSnapshotSamples in lib/snapshotPlayer.js).
export function sampleUrlsForSynth(synthType, params = {}) {
  if (synthType === 'Drums') {
    const voice = DRUM_VOICES.find(v => v.id === params.drumVoice) ?? DRUM_VOICES[0]
    return [DRUM_BASE_URL + voice.file]
  }
  if (synthType === 'Sampler') {
    const preset = SAMPLER_PRESETS[params.samplerPreset] ?? SAMPLER_PRESETS.piano
    return Object.values(preset.urls).map(file => preset.baseUrl + file)
  }
  return []
}

// One sample is enough for the grain source — see granularSourceZone
// (soundSpecs.js) for why its real note name has to travel with it.
async function fetchGranularRenderSample(synthType, params) {
  const zone = granularSourceZone(synthType, params, GRANULAR_RENDER_NOTE)
  return { buffer: await loadSample(zone.url), note: zone.note }
}
...
function buildSynthOpts(synthType, params = {}, volume) {
  ...
  switch (synthType) {
    ...
    case 'MonoSynth': return {
      ...vol, detune: p.detune ?? 0,
      oscillator: osc, envelope: env,
      // No filter.frequency: Tone connects filterEnvelope into it and zeroes its
      // base value, so the cutoff sweeps baseFrequency → baseFrequency·2^octaves.
      // Passing a frequency made every later set() add it back as an offset —
      // the same patch sounded brighter after an edit than after a reload.
      filter: { type: p.filterType, rolloff: p.filterRolloff, Q: p.filterQ },
      filterEnvelope: {
    ...
    // Sample-backed voices resolve their zone map through lib/sampleCache.js:
    // any sample already decoded is handed over as an AudioBuffer, so the Sampler
    // is `loaded` (i.e. audible) the instant it's constructed instead of after a
    // round of downloads. Uncached zones fall back to absolute URL strings, which
    // is exactly the old behaviour — hence baseUrl: ''.
    case 'Sampler': {
      const preset = SAMPLER_PRESETS[p.samplerPreset] ?? SAMPLER_PRESETS.piano
      return {
        ...vol, urls: resolveSamplerUrls(preset.urls, preset.baseUrl), baseUrl: '',
    ...
    case 'Drums': {
      ...
      return {
        ...vol,
        urls: resolveSamplerUrls({ [DRUM_TRIGGER_NOTE]: voice.file }, DRUM_BASE_URL),
        baseUrl: '',
    ...
    case 'DuoSynth': return {
      ...
      voice1: { oscillator: { type: p.voice1OscType ?? p.voice0OscType }, envelope: env },
    }
...
export const SYNTH_PARAM_TARGETS = {
  ...
  MonoSynth: [
    ...
    { id: 'synth.filterEnvBaseFreq', label: 'Flt Base',   group: 'Filter Env', min: 20,    max: 5000, curve: 'exp' },
  ...
  NoiseSynth: [],
  PluckSynth: [
    // attackNoise is the pick-noise burst in string periods; at 0 there is no
    // excitation at all and the string is silent.
    { id: 'synth.attackNoise', label: 'Attack Noise', group: 'Pluck', min: 0.1, max: 20 },
    { id: 'synth.dampening',   label: 'Dampening',    group: 'Pluck', min: 200, max: 8000, curve: 'exp' },
...
// Live-set amp-envelope keys. DuoSynth has no top-level `envelope` — Tone's
// set() silently skips unknown keys, so its env automation used to be a no-op —
// and its amp envelopes live on the two MonoSynth voices. Sample-backed voices
// keep attack/release top-level and have no decay/sustain.
function setAmpEnvelope(synth, synthType, env) {
  if (synthType === 'Sampler' || synthType === 'Drums') {
    const { attack, release } = env
    const live = {}
    if (attack != null) live.attack = attack
    if (release != null) live.release = release
    if (Object.keys(live).length) synth.set(live)
  } else if (synthType === 'DuoSynth') {
    synth.set({ voice0: { envelope: env }, voice1: { envelope: env } })
  } else {
...
function applySynthParam(synth, synthType, paramKey, value) {
  try {
    switch (paramKey) {
      ...
      case 'decay':
      case 'sustain':
      case 'release':             setAmpEnvelope(synth, synthType, { [paramKey]: value }); break
      ...
      case 'filterEnvBaseFreq':   synth.set({ filterEnvelope: { baseFrequency: value } }); break
      ...
      case 'resonance':
        // Tone 15's PluckSynth.resonance is a plain number read at each
        // triggerAttack, not a Signal — the old `'value' in synth.resonance`
        // threw on a number (swallowed below), so automation never landed.
        if (synthType === 'PluckSynth') synth.resonance = value
...
export class TransitEngine {
  constructor(onEvent) {
    ...
    // Per-route chromatic transpose in semitones: routeId → integer offset.
    // Applied after the octave shift in _buildRoutePart; set when a lane is
    // duplicated with a transpose. Free chromatic move (may leave the key).
    this._semitoneShifts = {}
    ...
    // Per-route pitch variety: routeId → { contour, variety } (DEFAULT_PITCH_VARIETY
    // shape). Feeds generatePitchMap opts when the Part is built; persists across
    // start/stop like the other per-route config maps.
    this._pitchVariety = {}

    // Per-stop authored velocities: routeId → { stopId: 0.2..1 }. Overrides the
    // variety-derived velocity map for that stop when the Part fires.
    this._stopVelocities = {}

    // Note chance + loop rest pattern (lib/laneGating.js). Read at callback time,
    // never baked into a Part, so editing them doesn't restart the lane.
    //   _noteChances:  routeId → 0..1 (lane-wide)
    //   _stopChances:  routeId → { stopId: 0..1 } (per-stop override)
    //   _loopPatterns: routeId → { play, rest, offset }
    //   _partLoopTicks / _partStartTicks: routeId → the lane Part's loop length
    //                  and anchor, so the callback can tell which pass of its own
    //                  loop it is on. Ticks, not seconds: a Part keeps its loop in
    //                  ticks, so a BPM change must not shift the count.
    this._noteChances    = {}
    this._stopChances    = {}
    // Per-lane note length (lib/noteLength.js): routeId → Tone division. Absent =
    // the legacy gate ('4n' mock, '8n' live). Callback-time read, like chance.
    this._noteLengths    = {}
    this._loopPatterns   = {}
    this._partLoopTicks  = {}
    this._partStartTicks = {}
    ...
    this._routeEqs     = {}     // routeId → weq8 spec (8-tuple of filter objs); legacy EQ3 objs migrated on load
    this._eqRuntimes   = {}     // routeId → WEQ8Runtime (live audio graph for the EQ)
    this._onRouteEqChange = null // (routeId, spec) => void — UI mirror for autosave
    ...
    // Per-route sidechain ducking config (DEFAULT_SIDECHAIN shape). Persists
    // across start/stop; the duck Gain node lives on the route entry.
    this._sidechains = {}
    // Reverse index sourceKey → Set(destRouteId), rebuilt by setSidechain. Keeps
    // the note-trigger hot path to one size check when nothing is configured.
    this._sidechainIndex = new Map()
    ...
    this._drumSeq     = null
    this._drumInsert  = null   // _mockSynths entry for DRUMS_ROUTE_ID (built in _startDrumSeq)
  ...
  init() {
    // Per-engine master fader, ahead of the shared Destination. Two engines can be
    // alive at once (the Song Chainer hands off between an A and a B engine across
    // an item boundary), and Tone's one Destination volume can't fade them apart.
    this._masterGain = new Tone.Gain(1).toDestination()
    this._alertLayer = new AlertLayer(this._masterGain)
    this._panners = {}
  ...
  // Per-route chromatic transpose (semitones). Read at trigger time, but rebuild
  // any running Part so a stopped-then-edited value takes effect immediately.
  setSemitoneShift(routeId, semis) {
    this._semitoneShifts[routeId] = semis || 0
  ...
  // Per-stop authored velocities for a lane: { stopId: 0.2..1 }. Read at trigger
  // time inside the Part callback, but rebuild so the closure captures the new map.
  setStopVelocities(routeId, map) {
    this._stopVelocities[routeId] = map ?? {}
    if (this._routeParts[routeId]) this._rebuildRoutePart(routeId)
  }

  // Lane-wide note chance (0..1). Callback-time read — no Part rebuild.
  setNoteChance(routeId, chance) {
    if (chance == null) delete this._noteChances[routeId]
    else this._noteChances[routeId] = normalizeNoteChance(chance)
  }

  // How long a lane's ordinary stop notes are held ('8n', '2n', …); null resets
  // to the legacy gate. Callback-time read — no Part rebuild.
  setNoteLength(routeId, length) {
    const len = normalizeNoteLength(length)
    if (len) this._noteLengths[routeId] = len
    else delete this._noteLengths[routeId]
  }

  // Per-stop chance overrides: { stopId: 0..1 }. Callback-time read.
  setStopChances(routeId, map) {
    this._stopChances[routeId] = map ?? {}
  }

  // Loop rest pattern: { play, rest, offset }. Callback-time read.
  setLoopPattern(routeId, pattern) {
    if (!pattern) delete this._loopPatterns[routeId]
    else this._loopPatterns[routeId] = normalizeLoopPattern(pattern)
  }

  // Which pass of its own loop a lane is on at audio-context time `time`
  // (defaults to now), and whether its loop pattern rests there. Mock-only.
  _laneLoopIndex(routeId, time) {
    const loopTicks = this._partLoopTicks[routeId]
    if (!loopTicks) return 0
    let ticks
    try { ticks = Tone.Transport.getTicksAtTime(time ?? Tone.now()) } catch { return 0 }
    return loopIndexAt(ticks, this._partStartTicks[routeId] ?? 0, loopTicks)
  }

  // Converted exactly as Part.loopEnd / Part.start convert them (at build-time BPM).
  _recordPartLoop(routeId, partLoopSec) {
    this._partLoopTicks[routeId]  = Tone.Time(partLoopSec).toTicks()
    this._partStartTicks[routeId] = Tone.Time(this._partStartAt ?? 0).toTicks()
  }

  getRouteLoopState(routeId) {
    if (!this._routeParts[routeId]) return null
    const loopIndex = this._laneLoopIndex(routeId)
    const pattern   = this._loopPatterns[routeId]
    return { loopIndex, resting: pattern ? !loopPlays(loopIndex, pattern) : false }
  }

  // Decide one scheduled lane note: 'play', 'rest' (the loop pattern sits this
  // pass out) or 'drop' (the chance roll failed). Pattern first — a resting
  // loop shouldn't spend a roll. stopId is omitted for merged chords.
  _laneGate(routeId, time, stopId) {
    const pattern = this._loopPatterns[routeId]
    if (pattern && pattern.rest > 0 && !loopPlays(this._laneLoopIndex(routeId, time), pattern)) return 'rest'
    const stopChance = stopId != null ? this._stopChances[routeId]?.[stopId] : undefined
    return rollChance(resolveNoteChance(this._noteChances[routeId], stopChance)) ? 'play' : 'drop'
  }

  // A legato lane holds its note until the next attack; entering a rest loop
  // must let go, or the last note drones through the whole rest.
  _releaseRestingLegato(entry, routeId, time) {
    if (!this._legatoRoutes[routeId] || entry._restReleased) return
    entry._restReleased = true
    for (const syn of [entry.synth, entry.harmonySynth]) {
      if (!syn) continue
      try {
        if (typeof syn.releaseAll === 'function') syn.releaseAll(time)   // PolySynth / Sampler
        else syn.triggerRelease?.(time)
      } catch {}
    }
  }
  ...
  addRoute(route, soundMode = { mode: 'harmonic', scale: { root: 'C', scaleType: 'major' } }, synthType = 'Synth', envelope = null) {
    ...
    if (this._routes && !this._routes.find(r => r.id === route.id)) {
  ...
  removeRoute(routeId) {
    ...
    this._disposeEqRuntime(routeId)
    ...
    for (const m of [this._pitchOffsets, this._pitchVariety, this._stopVelocities, this._noteChances, this._noteLengths, this._stopChances, this._loopPatterns, this._partLoopTicks, this._partStartTicks, this._octaveShifts, this._semitoneShifts, this._glides, this._legatoRoutes,
                     this._arpeggiators, this._granulars, this._sidechains, this._routeVolumesDb, this._routePans,
                     this._routeDisabled, this._routeFilters, this._routeEqs, this._eqRuntimes, this._droneRoutes,
                     this._trackSpeeds, this._trackLoopRegions, this._gridResolutions, this._cachedSoundModes,
                     this._merges]) {
      delete m[routeId]
    }
    // Any lane sidechained *from* this one loses its trigger rather than ducking
    // forever off a source that no longer fires.
    for (const [rid, cfg] of Object.entries(this._sidechains)) {
      if (cfg?.source !== routeId) continue
      cfg.source = ''
      cfg.enabled = false
      this._releaseDuck(rid)
    }
    this._rebuildSidechainIndex()
  ...
  _applyRouteGain(routeId) {
    ...
    const soloedElsewhere = this._soloRoutes.size > 0 && !this._soloRoutes.has(routeId)
    const disabled = this._routeDisabled[routeId] || soloedElsewhere
  ...
  // Register a listener that fires whenever a route's weq8 EQ changes (the curve
  // editor mutates the runtime directly). The UI mirrors this into React state so
  // autosave/persistence stays in sync. Only one listener is supported.
  setOnRouteEqChange(cb) {
    this._onRouteEqChange = cb
  }

  // The live WEQ8Runtime for a route, creating it on demand. It persists across
  // start/stop (independent of the mock synth entry) so the curve editor stays
  // usable while stopped; the route entry connects it into the audio chain when
  // playback (re)builds. normalizeEqState hands it a fresh migrated spec clone.
  getRouteEqRuntime(routeId) {
    let rt = this._eqRuntimes[routeId]
    if (rt) return rt
    const spec = normalizeEqState(this._routeEqs[routeId])
    rt = new WEQ8Runtime(this.getAudioContext(), spec)
    rt._leidUnsub = rt.on('filtersChanged', (s) => {
      this._routeEqs[routeId] = s
      this._onRouteEqChange?.(routeId, cloneSpec(s))
    })
    this._eqRuntimes[routeId] = rt
    this._routeEqs[routeId]   = rt.spec
    return rt
  }

  // Tear down a route's EQ runtime (route removal / engine dispose).
  _disposeEqRuntime(routeId) {
    const rt = this._eqRuntimes[routeId]
    if (!rt) return
    try { rt._leidUnsub?.() } catch {}
    try { rt.input?.disconnect() } catch {}
    delete this._eqRuntimes[routeId]
  }

  // Apply a full serialized weq8 spec to a route (snapshot replay, duplication,
  // migration). Legacy EQ3 objects are migrated by normalizeEqState. Stored for
  // lazy creation and applied live if the runtime already exists.
  setRouteEqState(routeId, state) {
    const spec = normalizeEqState(state)
    this._routeEqs[routeId] = spec
    const rt = this._eqRuntimes[routeId]
    if (!rt) return
    for (let i = 0; i < spec.length; i++) {
      const f = spec[i]
      rt.setFilterType(i, f.type)
      rt.toggleBypass(i, !!f.bypass)
      if (Number.isFinite(f.frequency)) rt.setFilterFrequency(i, f.frequency)
      if (Number.isFinite(f.Q))         rt.setFilterQ(i, f.Q)
      if (Number.isFinite(f.gain))      rt.setFilterGain(i, f.gain)
    }
  }
  ...
  setSendLevel(instRouteId, fxBusId, level) {
    ...
    if (!entry?.routePanner || !fxBus) return

    const sendGain = new Tone.Gain(level)
    entry.routePanner.connect(sendGain)
  ...
  _buildAutomationLanePart(key) {
    ...
    part.start(this._partStartAt ?? 0)
    return part
  }
  ...
  _ensureSendGain(routeId, fxBusId) {
    ...
    const entry = this._mockSynths.get(routeId)
    const fxBus = this._fxTracks[fxBusId]
    if (!entry?.routePanner || !fxBus) return null
  ...
  _applyAutomation(routeId, paramTarget, normalizedValue, glide) {
    ...
    if (paramTarget.startsWith('send.')) {
      ...
      try { if (entry?.synth) setAmpEnvelope(entry.synth, entry.synthType, { [param]: v }) } catch {}
  ...
  _triggerSynth(entry, note, dur, time, velocity = 1) {
    ...
    if (synthType === 'Drums') {
      ...
      if (synth.loaded) synth.triggerAttackRelease(DRUM_TRIGGER_NOTE, dur, time, velocity)
    } else if (synthType === 'Sampler') {
      if (synth.loaded) synth.triggerAttackRelease(note, dur, time, velocity)
      ...
      if (harmonySynth && harmonyInterval) {
        harmonySynth.triggerAttackRelease(
          Tone.Frequency(note).transpose(harmonyInterval).toFrequency(), dur, time, velocity
        )
      }
    }
    // Layer a granular burst of the rendered instrument at the same pitch.
    // Merged (PolySynth) lanes pass a chord array here — the mono grain voice
    // takes a single pitch, so granulate the chord root.
    const g = entry.granularVoice
    if (g?.loaded && this._granulars[entry.routeId]?.enabled) {
      g.triggerAttackRelease(Array.isArray(note) ? note[0] : note, dur, time)
    }
  }

  _triggerLegatoNote(entry, note, time, velocity = 1) {
    const { synth, synthType, harmonySynth, harmonyInterval } = entry
    ...
    if (synthType === 'NoiseSynth') {
      ...
      if (synth.loaded) synth.triggerAttack(DRUM_TRIGGER_NOTE, time, velocity)
    } else if (synthType === 'Sampler') {
      if (synth.loaded) synth.triggerAttack(note, time, velocity)
    } else {
      try { synth.triggerAttack(note, time, velocity) } catch {}
      if (harmonySynth && harmonyInterval) {
        try {
          harmonySynth.triggerAttack(
            Tone.Frequency(note).transpose(harmonyInterval).toFrequency(), time, velocity
          )
        } catch {}
      }
    }
  ...
  _triggerArp(entry, routeId, rootNote, time, velocity = 1) {
    ...
    seq.forEach((n, i) => this._triggerSynth(entry, n, dur, time + i * stepSec, velocity))
  }

  // Arpeggiate a merged (PolySynth) lane's chord: rather than sound the stacked
  // notes together, sequence the chord's own notes at the synced rate. Reuses
  // the same rate/gate timing as _triggerArp; ordering/octaves come from the cfg.
  _triggerChordArp(entry, routeId, chordNotes, time) {
    const cfg = this._arpeggiators[routeId]
    if (!cfg) return
    const seq = buildChordArpSequence(chordNotes, cfg)
    const stepSec = Tone.Time(cfg.rate ?? '16n').toSeconds()
    const dur = stepSec * (cfg.gate ?? 0.5)
  ...
  _disposeRouteEntry(entry) {
    ...
    // Cleanly remove the weq8 EQ's output→duck edge before disposing the duck node,
    // but keep the runtime alive (it persists across start/stop, owned by _eqRuntimes).
    try { entry.eq?.disconnect(entry.duckGain?.input) } catch {}
    entry.duckGain?.dispose()
  ...
  _createSingleRouteEntry(routeId, routeType, synthType = 'Synth', envelope = null, scale = null) {
    ...
    // Sidechain duck stage — see DEFAULT_SIDECHAIN. Always built (a unity GainNode
    // is free) and deliberately NOT routeGain, which already has three writers:
    // manual volume, solo/disable, and `volume` automation.
    ...
    // Per-route weq8 parametric EQ (raw Web Audio, bridged into the Tone chain).
    // The runtime persists across start/stop (owned by _eqRuntimes, not the entry)
    // so the curve editor stays live while stopped; here we just wire it in.
    const eq = this.getRouteEqRuntime(routeId)

    synth.connect(routeGain)
    routeGain.connect(filter)
    filter.connect(eq.input)          // Tone.Filter → native GainNode (weq8 input)
    eq.connect(duckGain.input)        // weq8 output → native GainNode
    duckGain.connect(routePanner)
    ...
    const entry = {
      ...
      routeGain, routePanner, filter, eq, duckGain,
    }
  ...
  // Per-route pitch variety: { contour, variety } (see PITCH_CONTOURS /
  // DEFAULT_PITCH_VARIETY in mappings.js). Mock-only — live arrivals pitch each
  // note per-arrival via computeNote, which doesn't consult this map.
  setPitchVariety(routeId, cfg) {
    this._pitchVariety[routeId] = { ...DEFAULT_PITCH_VARIETY, ...(cfg ?? {}) }
    if (this._routeParts[routeId]) this._rebuildRoutePart(routeId)
  }
  ...
  // line-type bus / master reverb — i.e. a dry channel stem. The FX sends
  // branch off this same node (post-insert), but their bus returns are not
  // part of the stem.
  ...
  // This engine's own master fader (see init). Fading it rather than the shared
  // Destination is what lets an outgoing engine ring out under an incoming one.
  getMasterGain() {
  ...
  _recordMidiNote(routeId, note) {
    ...
    this._midiRecorder.record({
      ...
      noteLength: this._noteLengths[routeId] ?? null,
    })
  }
  ...
  setDrumPattern(pattern) {
    ...
    if (this._drumPattern) {
      if (this._drumSeq) this._drumSeq.load(this._drumPattern)
      else if (Tone.Transport.state === 'started') this._startDrumSeq()
      return
    }
    // Cleared: stop + tear down the sequencer and its insert chain.
    if (this._drumSeq) { this._drumSeq.dispose(); this._drumSeq = null }
    this._disposeDrumInsert()
  }

  // Build + schedule the drum sequencer on the current Transport (no-op if there
  // is no pattern or it has no active steps). Feeds the drum insert chain so the
  // lane's volume/filter/eq/sends apply and master/WAV export capture the drums.
  _startDrumSeq() {
    if (this._drumSeq || !this._drumPattern) return
    const insert = this._ensureDrumInsert()
    const seq = new DrumSequencer()
    seq.init(insert.routeGain)
    seq.load(this._drumPattern)
    // Per-hit audio-rate hook so lanes can duck off the beat. Fires with the raw
    // scheduler time (ahead of the audio clock) and skips muted pads.
    seq.setOnTrigger((padId, time, vel) => {
      this._fireSidechain([DRUMS_ROUTE_ID, `${DRUMS_ROUTE_ID}:${padId}`], time, vel)
    })
    if (!seq.hasActiveSteps()) { seq.dispose(); return }
    seq.schedule(this._partStartAt ?? 0)
    this._drumSeq = seq
  }

  // Build the drum lane's insert chain (routeGain → filter → eq → panner → master)
  // under DRUMS_ROUTE_ID, mirroring _createSingleRouteEntry but fed by the drum
  // sequencer instead of a synth. Idempotent. Reads persisted per-id mixer state so
  // volume/filter/eq/sends set while stopped apply on play. Disposed by stopMock
  // (generic _mockSynths teardown) like any route entry.
  _ensureDrumInsert() {
    if (this._drumInsert) return this._drumInsert
    const rid     = DRUMS_ROUTE_ID
    const lineOut = this._alertLayer?.input ?? this.getMasterOutputNode()

    const db          = this._routeVolumesDb[rid] ?? 0
    const soloedOut   = this._soloRoutes.size > 0 && !this._soloRoutes.has(rid)
    const initGain    = (this._routeDisabled[rid] || soloedOut) ? 0 : Math.pow(10, db / 20)
    const initPan     = this._routePans[rid] ?? 0

    const routeGain   = new Tone.Gain(initGain)
    const routePanner = new Tone.Panner(initPan)
    // Kept identical to _createSingleRouteEntry so the drum lane can be a
    // sidechain destination too — that only needs UI, no plumbing.
    const duckGain    = new Tone.Gain(1)
    const fParams     = this._routeFilters[rid] ?? { type: 'lowpass', frequency: 20000, Q: 4 }
    const filter      = new Tone.Filter(fParams)
    const eq          = this.getRouteEqRuntime(rid)

    routeGain.connect(filter)
    filter.connect(eq.input)
    eq.connect(duckGain.input)
    duckGain.connect(routePanner)
    routePanner.connect(lineOut)

    // Wire any pending sends (buses exist by the time _startDrumSeq runs).
    for (const [key, level] of Object.entries(this._pendingSends)) {
      const [instId, fxBusId] = key.split(':')
      if (instId !== rid) continue
      const fxBus = this._fxTracks[fxBusId]
      if (!fxBus || this._sendGains[key]) continue
      const sendGain = new Tone.Gain(level)
      routePanner.connect(sendGain)
    ...
    const entry = { routeId: rid, routeType: null, synthType: null, routeGain, routePanner, filter, eq, duckGain }
    this._mockSynths.set(rid, entry)
    this._drumInsert = entry
    return entry
  }

  // Tear down the drum insert chain + its sends (drum lane removed mid-playback).
  _disposeDrumInsert() {
    if (this._drumInsert) {
      this._disposeRouteEntry(this._drumInsert)
      this._mockSynths.delete(DRUMS_ROUTE_ID)
      this._drumInsert = null
    }
    for (const key of Object.keys(this._sendGains)) {
      if (key.startsWith(`${DRUMS_ROUTE_ID}:`)) { this._sendGains[key]?.dispose(); delete this._sendGains[key] }
    }
  }

  // opts.startAt — the Transport time every Part is anchored to (default 0, i.e.
  // "the transport's origin", which is what a fresh Transport gives you). The Song
  // Chainer passes a *future* time so a whole section can be built and scheduled
  // ahead of its boundary and simply begin sounding when the transport reaches it —
  // that's what makes an item swap gapless instead of a teardown-and-rebuild stall.
  // Because Parts loop on their own length from this anchor, a section still starts
  // at its loop origin no matter where the shared transport happens to be.
  startMock(routes, soundModes = {}, bpm = 120, synthTypes = {}, adsr = {}, effects = {}, opts = {}) {
    Tone.Transport.bpm.value = bpm
    this._partStartAt = opts.startAt ?? 0

    // Keep an engine-owned array. MixerTab may pass its React route state here,
    // which must never be mutated by hot add/remove operations.
    this._routes = [...routes]
    ...
    // Never restart a transport another engine is already riding (A/B handoff).
  ...
  _buildMergedRoutePart(route) {
    ...
    const semitone  = this._semitoneShifts[route.id] ?? 0

    // Bucket every source's in-region stop notes by grid cell → one chord per cell.
    // The merged lane's variety cfg applies to every source, seeded per source id
    // so the stacked voices stay distinct.
    const varietyCfg = this._pitchVariety[route.id] ?? DEFAULT_PITCH_VARIETY
    const chords = new Map()   // cellIdx → { notes: Set<string>, name: string }
    for (const src of sources) {
      const pitchMap  = generatePitchMap(src.stops, rootMidi, modeScale, 3,
        { ...varietyCfg, routeId: src.id })
      const gridStops = snapStopsToGrid(src.stops, src.totalDist, noteTotalCells, stepsPerBar)
        .filter(s => {
          const frac = s.cellIdx / noteTotalCells
          return frac >= regionStartFrac && frac < regionEndFrac
      ...
      for (const stop of gridStops) {
        ...
        const note = shiftSemitones(shiftOctaveNote(raw, octave), semitone)
    ...
    const part = new Tone.Part((time, ev) => {
      ...
      if (!e) return
      const gate = this._laneGate(route.id, time)   // lane chance only — one roll per chord
      if (gate === 'rest') { this._releaseRestingLegato(e, route.id, time); return }
      e._restReleased = false
      if (gate === 'drop') return
      if (this._arpeggiators[route.id]?.enabled) {
        // Chord arpeggiator: sequence the merged notes instead of stacking them.
        this._triggerChordArp(e, route.id, ev.notes, time)
      } else {
        // Tone.PolySynth.triggerAttackRelease accepts an array → the whole chord in one call.
        this._triggerSynth(e, ev.notes, this._noteLengths[route.id] ?? noteDur, time)
      }
      ...
      this._fireSidechain(route.id, time)   // one duck per chord, not per note
    }, events)

    part.loop    = true
    part.loopEnd = partLoopSec
    this._recordPartLoop(route.id, partLoopSec)
    part.start(this._partStartAt ?? 0)
    return part
  }
  ...
  _buildRoutePart(route) {
    ...
    // longitude → octave register (see generatePitchMap / geoToMidi), plus the
    // track's pitch-variety opts (contour + seeded jitter) and the matching
    // gap-driven velocity map.
    ...
    const varietyCfg    = this._pitchVariety[route.id] ?? DEFAULT_PITCH_VARIETY
    const pitchMap      = generatePitchMap(route.stops, autoRootMidi, autoModeScale, 3,
      { ...varietyCfg, routeId: route.id })
    const velocityMap   = generateVelocityMap(route.stops, varietyCfg.variety)
    ...
    const part = new Tone.Part((time, stop) => {
      ...
      const e = this._mockSynths.get(route.id)
      if (!e) return
      // Loop pattern + note chance. Bail before onEvent / MIDI record / sidechain,
      // so a silent note neither flashes the map nor ducks another lane.
      const gate = this._laneGate(route.id, time, stop.id)
      if (gate === 'rest') { this._releaseRestingLegato(e, route.id, time); return }
      e._restReleased = false
      if (gate === 'drop') return
      ...
      const octaved   = shiftOctaveNote(tuned, this._octaveShifts[route.id] ?? 0)
      const note = shiftSemitones(octaved, this._semitoneShifts[route.id] ?? 0)
      const velocity = this._stopVelocities[route.id]?.[stop.id] ?? velocityMap[stop.originalIdx] ?? 1
      if (this._arpeggiators[route.id]?.enabled) {
        this._triggerArp(e, route.id, note, time, velocity)
      } else if (this._legatoRoutes[route.id]) {
        this._triggerLegatoNote(e, note, time, velocity)
      } else {
        this._triggerSynth(e, note, this._noteLengths[route.id] ?? noteDur, time, velocity)
      }
      this.onEvent({ routeShortName: route.name, stopName: stop.name, note, lineType: route.type })
      this._recordMidiNote(route.id, note)
      // One duck per stop event, not per note — hooking _triggerSynth instead
      // would machine-gun the envelope once the lane is arpeggiated.
      this._fireSidechain(route.id, time, velocity)
    }, gridStops.map(stop => [
      ((stop.cellIdx / noteTotalCells - regionStartFrac) / (regionEndFrac - regionStartFrac)) * partLoopSec,
      stop,
    ]))
  ...
  triggerLiveNote(routeId, routeType, note) {
    ...
    if (!e) return
    // Live arrivals have no loop to rest on — only the lane's note chance applies.
    if (!rollChance(normalizeNoteChance(this._noteChances[routeId]))) return
    ...
    if (this._arpeggiators[routeId]?.enabled) {
      ...
      this._triggerSynth(e, note, this._noteLengths[routeId] ?? '8n', time)
    }
    this.onEvent({ routeShortName: routeId, note, lineType: routeType })
    this._recordMidiNote(routeId, note)
    this._fireSidechain(routeId, time)
  }

  setSolo(routeId, isSoloed) {
    if (isSoloed) this._soloRoutes.add(routeId)
    else          this._soloRoutes.delete(routeId)
    this._refreshSoloGains()
  }

  // Drop every solo. Solo membership is additive (setSolo only adds), so loading a
  // song has to clear first — otherwise the previous song's solos persist, and any
  // solo at all gates every non-soloed lane silent.
  clearSolos() {
    if (!this._soloRoutes.size) return
    this._soloRoutes.clear()
    this._refreshSoloGains()
  }

  // Re-apply gain everywhere the solo set affects. Note triggers gate on
  // _soloRoutes too (see the mock Part callbacks), but sustained voices — drones,
  // granular layers, a held legato note — only follow the gain, so they need this.
  _refreshSoloGains() {
    for (const rid of this._mockSynths.keys()) this._applyRouteGain(rid)
    // Drums aren't solo-able and are never in _soloRoutes, but they must still
    // duck when another lane is soloed; their Sequence bypasses the trigger gate.
    this._applyRouteGain(DRUMS_ROUTE_ID)
  }
  ...
  updateEnvelope(routeId, params) {
    ...
    if (e.synthType === 'Sampler' || e.synthType === 'Drums') {
      ...
      setAmpEnvelope(e.synth, e.synthType, params)
      return
    }
  ...
  // ── Sidechain ducking ─────────────────────────────────────────────────────────
  // See DEFAULT_SIDECHAIN. The duck Gain sits after the EQ and before the panner,
  // so the FX sends (which branch off the panner) pump with the lane and WAV stem
  // export — which taps routePanner via getRouteOutputNode — captures the effect.

  setSidechain(routeId, cfg) {
    const prev = this._sidechains[routeId] ?? {}
    const merged = { ...DEFAULT_SIDECHAIN, ...prev, ...cfg }
    this._sidechains[routeId] = merged
    this._rebuildSidechainIndex()
    // A node left mid-duck stays quiet forever, so turning the sidechain off has
    // to hand the gain back — the same reason _restoreParamToManual exists.
    if (!merged.enabled || !merged.source) this._releaseDuck(routeId)
  }

  _rebuildSidechainIndex() {
    const index = new Map()
    for (const [routeId, cfg] of Object.entries(this._sidechains)) {
      // A lane keyed off itself would duck on its own notes — never useful, and
      // the picker hides it, but a stale song could still carry one.
      if (!cfg?.enabled || !cfg.source || cfg.source === routeId) continue
      let set = index.get(cfg.source)
      if (!set) { set = new Set(); index.set(cfg.source, set) }
      set.add(routeId)
    }
    this._sidechainIndex = index
  }

  // Duck every lane listening to any of `keys` (a source key or array of them —
  // a drum hit matches both '__drums__' and '__drums__:<pad>'). `time` is an
  // audio-context timestamp, normally in the future.
  _fireSidechain(keys, time, velocity = 1) {
    if (this._sidechainIndex.size === 0) return
    const list = Array.isArray(keys) ? keys : [keys]
    let fired = null
    for (const key of list) {
      const targets = this._sidechainIndex.get(key)
      if (!targets) continue
      for (const routeId of targets) {
        // 'any pad' and 'this pad' can both match one hit — duck once.
        if (fired?.has(routeId)) continue
        (fired ??= new Set()).add(routeId)
        this._duck(routeId, time, velocity)
      }
    }
  }

  _duck(routeId, time, velocity = 1) {
    const param = this._mockSynths.get(routeId)?.duckGain?.gain
    if (!param) return
    const cfg = this._sidechains[routeId]
    if (!cfg) return
    const depth = 1 - Math.pow(10, (cfg.amountDb ?? DEFAULT_SIDECHAIN.amountDb) / 20)
    // Velocity scales the dip, so a soft hat pushes less than a full kick. At the
    // default velocity of 1 the floor is exactly what the amount slider says.
    const floor = Math.max(0, 1 - depth * Math.max(0, Math.min(1, velocity)))
    const attack  = Math.max(0.001, cfg.attack  ?? DEFAULT_SIDECHAIN.attack)
    const release = Math.max(0.001, cfg.release ?? DEFAULT_SIDECHAIN.release)
    try {
      // Hold whatever the in-flight ramp is worth at `time` rather than jumping,
      // so a retrigger mid-release doesn't click.
      param.cancelAndHoldAtTime(time)
    } catch {
      param.cancelScheduledValues(time)
      param.setValueAtTime(param.value, time)
    }
    param.linearRampToValueAtTime(floor, time + attack)
    param.linearRampToValueAtTime(1, time + attack + release)
  }

  _releaseDuck(routeId) {
    const param = this._mockSynths.get(routeId)?.duckGain?.gain
    if (!param) return
    const now = Tone.now()
  ...
  async _renderGranularSource(routeId) {
    ...
    try {
      ...
      let sample = null
      if (synthType === 'Sampler' || synthType === 'Drums') {
        sample = await fetchGranularRenderSample(synthType, params)
        if (entry._grainRenderId !== renderId || this._mockSynths.get(routeId) !== entry) return
      }
      const rendered = await Tone.Offline(() => {
        const s = sample
          ? new Tone.Sampler({
              urls: { [sample.note]: sample.buffer },
  ...
  // keepTransport — tear down only this engine's own nodes and leave the shared
  // Transport running. Required for the Song Chainer's A/B handoff: cancel() clears
  // *every* callback on the transport, so an outgoing engine stopping the normal way
  // would silently unschedule the incoming engine's already-built Parts. Safe because
  // everything this engine schedules is owned by an object disposed below — the Parts
  // unschedule themselves, and DrumSequencer.clearSchedule() clears its own repeat id.
  stopMock({ keepTransport = false } = {}) {
    ...
    this._drumInsert = null   // disposed above with the rest of _mockSynths
  ...
  dispose() {
    ...
    this._voices.clear()
    for (const rid of Object.keys(this._eqRuntimes)) this._disposeEqRuntime(rid)

...
// Major Budapest interchange hubs — used for hub convergence chords. Other
// cities can override these via NetworkState#setHubs (e.g. auto-derived from the
// loaded route network). Defaults keep Budapest's curated interchanges.
const DEFAULT_HUBS = [
...
export class NetworkState {
  constructor(outputNode) {
    ...
    // ── Hub PolySynths ────────────────────────────────────────────────────────
    this._hubs = DEFAULT_HUBS
    this._buildHubSynths()
  ...
  // (Re)build the per-hub PolySynths from this._hubs.
  _buildHubSynths() {
    this._hubSynths = this._hubs.map(() => {
      const ps = new Tone.PolySynth(Tone.Synth, {
        oscillator: { type: 'sine' },
        envelope: { attack: 2.0, decay: 0.5, sustain: 0.7, release: 4.0 },
        volume: -22,
      })
      ps.connect(this._out)
      return ps
    })
    this._hubActiveNotes = this._hubs.map(() => new Set())
  }

  // Replace the hub set (e.g. with interchanges derived from another city's
  // network). Disposes the old synths and rebuilds. No-op for empty input.
  setHubs(hubs) {

...
export const SYNTH_DEFAULTS = {
  ...
  MonoSynth: {
    ...
    sustain: 0.5,
    release: 0.8,  releaseCurve: 'exponential',
    filterType: 'lowpass', filterRolloff: -12, filterQ: 1,
...
// Every instrument a lane can play (engine/planContract re-export this as
// SYNTH_TYPES). Must hold exactly the keys of SYNTH_DEFAULTS — test/sound-policy
// pins that.
...
// The instruments the DAW's lane picker offers (desktop rack + phone lane sheet).
// The rest still play when a plan or an older song sets them.
export const PICKER_SYNTH_TYPES = ['Synth', 'FMSynth', 'NoiseSynth', 'PolySynth', 'Sampler', 'Drums']

// Picker options for a lane currently on `current`: a lane already on a hidden
// type keeps it listed, so the select shows the real instrument instead of
// silently displaying the first option.
export function pickerSynthTypes(current) {
  return current && !PICKER_SYNTH_TYPES.includes(current) && SYNTH_TYPES.includes(current)
    ? [...PICKER_SYNTH_TYPES, current]
    : PICKER_SYNTH_TYPES
}
...
export const TONE_SUPPORT = {
  ...
  MonoSynth:  ['oscillator', 'filterEnvelope', 'filterQ'],
  FMSynth:    ['oscillator', 'harmonicity', 'modulationIndex', 'modEnvelope'],
  AMSynth:    ['oscillator', 'harmonicity', 'modEnvelope'],
  PluckSynth: ['attackNoise', 'dampening', 'resonance'],
}

// A plan `tone` → the flat keys a lane's param block (trackADSRs, the object the
// synth editors write) uses. With a synthType, keys that instrument doesn't
// honour are left out: an edit that names no synthType is filtered against the
// lane's current instrument, and that matters because two instruments share a
// slot with different meanings — PluckSynth's `resonance` is comb feedback
// (0..1), MetalSynth's is a frequency in Hz.
export function toneToSynthParams(tone, synthType) {
  if (!tone) return {}
  const supported = synthType ? (TONE_SUPPORT[synthType] ?? []) : null
  const has = (key) => tone[key] != null && (!supported || supported.includes(key))
  const out = {}
  if (has('oscillator'))      out.oscillatorType  = tone.oscillator
  if (has('harmonicity'))     out.harmonicity     = tone.harmonicity
  if (has('modulationIndex')) out.modulationIndex = tone.modulationIndex
  if (has('modEnvelope')) {
  ...
  if (has('filterEnvelope')) {
    const f = tone.filterEnvelope
    if (f.attack != null)        out.filterEnvAttack   = f.attack
    if (f.decay != null)         out.filterEnvDecay    = f.decay
    if (f.sustain != null)       out.filterEnvSustain  = f.sustain
    if (f.release != null)       out.filterEnvRelease  = f.release
    if (f.baseFrequency != null) out.filterEnvBaseFreq = f.baseFrequency
    if (f.octaves != null)       out.filterEnvOctaves  = f.octaves
  }
  if (has('filterQ'))     out.filterQ     = tone.filterQ
  if (has('attackNoise')) out.attackNoise = tone.attackNoise
  if (has('dampening'))   out.dampening   = tone.dampening
  if (has('resonance'))   out.resonance   = tone.resonance
  return out
}

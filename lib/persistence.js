...
// 3: snapshots record their city (cityId) and their exact lane list (routeIds).
//    Before that, route ids — which are city-scoped — were saved with no city and
//    no lane list, so loading a song re-rolled a random lane selection.
// 4: the default pitch contour is 'demand', not 'geographic'. Lanes a song never
//    configured carry no trackPitchVariety entry, so older snapshots are pinned
//    to 'geographic' on load rather than silently re-pitched onto the new default.
...
export async function saveSong(song) {
  ...
  const res = await fetch(`/api/presets/${encodeURIComponent(song.id)}`, {
    ...
    body: JSON.stringify({
      ...
      // The server derives the city column from state.cityId; sent explicitly
      // only so the intent is visible in the request.
      cityId: song.state?.cityId ?? null,
      state: song.state,
    }),
  })
  return _json(res)

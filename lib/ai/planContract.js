...
// loaded lanes are the only routes the model may use.
...
// being inlined (a city can have 1,300+ lines).
...
ROUTES: take every routeId from the list_routes tool for this city — never invent ids. Filter by type (metro, tram, trolley, bus, hev) or search by line name. stopCount is a rough note-density hint; meanDemand (0..1) is how busy the line's stops are, which drives the default "demand" pitch contour.

WORKFLOW: call preview_song_plan first and read its "dropped" list (settings that were invalid and ignored) before saving. Nothing is played or saved by a preview.

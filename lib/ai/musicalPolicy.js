...
import { SOUND_POLICY_TEXT, soundRecipesText, DEFAULT_SOUND_IDS, SOUND_IDS } from './soundPolicy.js'
 ...
 * @returns {string} the universal musical + sound policy plus the mode paragraph(s)
 */
export function musicalPolicyText(mode = 'new') {
  const modes = mode === 'both' ? COMPOSE_MODES : [MODE_TEXT[mode] ? mode : 'new']
  return [POLICY_CORE, SOUND_POLICY_TEXT, ...modes.map(m => MODE_TEXT[m])].join('\n\n')
}
...
// `sounds` are the soundPolicy.js recipes that fit its roles (synthesis guide
// §4a), and `blueprint` (four recipes) is the synthesis guide's §6 beat build;
...
export const GENRE_RECIPES = [
  {
    ...
    blueprint: `Warm house beat (~122–124 BPM): house kit with restrained hats and reliable kick/clap; R1 bass, sparse and dry, ducked off drums:kick; R5 keys as the foreground, keeping the piano transient; optional R8 answer, quieter and slower — omit it if it crowds the keys. Shared FX: synthetic reverb around 1 s and a synced delay, wet 1, low sends; echo answers the upper part, never the bass. Edits: shorten the bass decay, brighten the keys slightly, bring in the quiet answer.`,
  },
  {
  ...
  },
  {
    ...
    sounds: ['R1', 'R14', 'R3', 'R9', 'R12'],
    text: `Hypnotic, drum-led techno: rhythm and sound colour are the hook. Firm four-on-the-floor kick, offbeat hats, sparse clap; a short, dry Synth low part (low filter, ducked by drums:kick); one narrow-range dry Synth or struck-FM cell (randomWalk, variety ≤ 0.15, 16n grid); an optional quiet filtered NoiseSynth tick on a rest pattern. Phrygian or minor. Controlled distortion or an autofilter on the cell, a small delay send; keep the bass dry. One drifting upper loop may run against the stable drums. Reject: every lane moving across two octaves under long reverb tails. Suggested edits: cell filter cutoff, distortion send, the answer's rest pattern.`,
    blueprint: `Hypnotic transit beat (~132 BPM): steady kit, its accents establishing the pulse; R1 bass with low variety and a controlled tail; R3 (or R12) as the principal repeated cell; R9 only if the kit needs a contrasting texture, quiet and intermittent. Shared FX: a little distortion on the cell and a synced delay; leave the third bus free. Edits: compare the cell's distortion send, thin the texture, move the cell's filter — keep its notes recognizable while the colour changes.`,
  },
  {
  ...
  },
  {
    ...
    blueprint: `Soft broken beat (~78 BPM): relaxed hip-hop kit with softer hats (straight grid — there is no swing); R5 keys as the foreground (or guitar-nylon in the same role); R2 bass only if the keys leave low-frequency room; a sparse R8 answer, never a second continuous lead. Shared FX: restrained chorus or bitcrusher on the upper part and a small synthetic room; pick two treatments. Edits: change the sample source, shorten the sample release, soften the high percussion.`,
  },
  {
  ...
  },
  {
    ...
    sounds: ['R1', 'R3', 'R12', 'R15'],
    text: `Broken kick (0 and 10), backbeat on 4 and 12, hats on 2,6,9,14 at mixed levels; short, syncopated bass notes (staccato envelope); sparse pluck-like responses (a short-envelope Synth, struck FMSynth or short Sampler). Keep reverb short so the rhythmic gaps stay audible. The composer has no swing/microtiming or vocal-chop control: call the result a garage-inspired straight-grid sketch — authentic shuffle is a capability gap, not something tempo fixes. Suggested edits: reduce overlapping melodic activity, shorten the bass release, the pluck's delay send.`,
  },
  {
    ...
    blueprint: `Atmospheric fast beat (~174 BPM): D&B kit skeleton; R1 or R2 bass with deliberate gaps and a release short enough for the fast kick; R4 or R5 foreground with slower activity than the percussion; R7 support only if it reinforces the tonal centre. Shared FX: synthetic room/plate reverb and a light delay on the upper part, bass dry; no grain at first. Edits: remove every unnecessary support event, shorten the bass tail, brighten the one focal instrument — fast drums do not need frantic melody.`,
  },
  {
...
export function recipeContextText(selection) {
  ...
  if (recipe.blueprint) lines.push(`BEAT BLUEPRINT — ${recipe.blueprint}`)
  if (secondary) {
...
/**
 * The SOUND RECIPES block for a prompt: the selected genre's sounds, or — with no
 * genre — the default palette (in-app) or every recipe (`all`, the MCP guide,
 * which is a reference read once rather than a per-request prompt).
 */
export function soundContextText(selection, { all = false } = {}) {
  if (selection?.recipe?.sounds?.length) return soundRecipesText(selection.recipe.sounds)
  return soundRecipesText(all ? SOUND_IDS : DEFAULT_SOUND_IDS)
}

...
import { NOTE_LENGTHS } from '../noteLength.js'
...
export function makePlanSchema({ strict = false } = {}) {
  ...
  const track = z.object({
    ...
    tone: opt(z.object({
      ...
      filterEnvelope: opt(z.object({
        attack: z.number(), decay: z.number(), sustain: z.number(), release: z.number(),
        baseFrequency: z.number(), octaves: z.number(),
      })),
      filterQ: opt(z.number()),
      attackNoise: opt(z.number()),
      dampening: opt(z.number()),
      resonance: opt(z.number()),
    })),
    filter: opt(filter),
    scale: opt(harmony),
    ...
    noteLength: opt(z.enum(NOTE_LENGTHS)),

...
import { buildReplacementLaneState, sendsToClear } from './planApply.js'
...
export function applyPlanToSnapshot(base, plan, { activeLaneLimit = null } = {}) {
  ...
  for (const key of [
    ...
    'trackPitchVariety', 'trackNoteChances', 'trackLoopPatterns', 'trackLabels', 'fxBusWet', 'fxBusParams', 'sendMatrix',
  ]) s[key] = map(base?.[key])
...
function laneDetail(snapshot, id) {
  ...
  if (adsr && adsr.attack != null) d.envelope = { attack: round2(adsr.attack), release: round2(adsr.release) }

...
// Shift a Tone.js note string by n semitones (chromatic): "D4" + 3 → "F4".
// Unlike shiftOctaveNote / transposeNoteInScale this is a free chromatic move
// and can leave the lane's key — used for whole-lane transpose of duplicates.
export function shiftSemitones(note, n) {
  if (!n) return note
  return midiToNote(noteToMidi(note) + n)
}
...
// Locate a MIDI note within a scale: the real tonic pitch at/below it, the scale
// interval array, and the note's scale-degree index (snapping DOWN to the nearest
// scale tone when the note isn't exactly on the scale). Shared by the arpeggiator
// and the diatonic transpose helpers below.
export function snapToScaleDegree(midi, scaleRoot = 'C', scaleType = 'major') {
  const iv = SCALES[scaleType] ?? SCALES.major
  const N  = iv.length
  const tonicPc    = Math.max(0, NOTE_NAMES.indexOf(scaleRoot))
  const relPc      = (((midi - tonicPc) % 12) + 12) % 12
  const tonicBelow = midi - relPc // a real tonic pitch at/below the note
  let baseDeg = iv.indexOf(relPc)
  if (baseDeg < 0) { baseDeg = 0; for (let d = 0; d < N; d++) if (iv[d] <= relPc) baseDeg = d }
  return { baseDeg, tonicBelow, iv, N }
}

// Scale-degree index (may be negative or span multiple octaves) → MIDI note.
function degreeToMidi(deg, tonicBelow, iv, N) {
  const wrapped = ((deg % N) + N) % N
  return tonicBelow + iv[wrapped] + 12 * Math.floor(deg / N)
}

// Transpose a note by `degrees` scale steps, staying diatonic (positive = up,
// negative = down). Returns a Tone.js note string. Used by the engine and the
// stop-rail note editor so duplicated lanes can only be re-pitched within key.
export function transposeNoteInScale(note, degrees = 0, scaleRoot = 'C', scaleType = 'major') {
  if (!degrees) return note
  const { baseDeg, tonicBelow, iv, N } = snapToScaleDegree(noteToMidi(note), scaleRoot, scaleType)
  return midiToNote(degreeToMidi(baseDeg + degrees, tonicBelow, iv, N))
}

// Given a drag target MIDI, return the integer scale-degree offset (relative to
// `baseNote`) of the nearest in-scale pitch — so a dragged note dot can never
// land off the scale. Searches degree offsets within ±2 octaves of the base.
export function nearestScaleDegreeOffset(baseNote, targetMidi, scaleRoot = 'C', scaleType = 'major') {
  const { baseDeg, tonicBelow, iv, N } = snapToScaleDegree(noteToMidi(baseNote), scaleRoot, scaleType)
  let best = 0, bestDist = Infinity
  for (let d = baseDeg - 2 * N; d <= baseDeg + 2 * N; d++) {
    const dist = Math.abs(degreeToMidi(d, tonicBelow, iv, N) - targetMidi)
    if (dist < bestDist) { bestDist = dist; best = d - baseDeg }
  }
  return best
}
...
export function buildArpSequence(rootNote, cfg = {}, scaleType = 'major', scaleRoot = 'C') {
  ...
  // Locate the stop note within the scale (snapping down to the nearest scale tone
  // if it isn't exactly on the scale).
  const { baseDeg, tonicBelow, iv, N } = snapToScaleDegree(noteToMidi(rootNote), scaleRoot, scaleType)
  ...
  for (let i = 0; i < nSteps; i++) {
    base.push(degreeToMidi(baseDeg + i * dist, tonicBelow, iv, N))
  }
...
// Arpeggiate an already-formed chord (a merged PolySynth lane's stacked notes):
// order the chord's own notes by `style` and optionally stack them across
// octaves. Unlike buildArpSequence, the notes are given — we don't synthesize
// chord tones from a single root — so `steps`/`distance` don't apply; only
// `octaves` + `style` shape the sequence (`rate`/`gate` are timing, applied by
// the engine).
export function buildChordArpSequence(chordNotes = [], cfg = {}) {
  const { octaves = 1, style = 'up' } = cfg
  const midi = [...new Set(chordNotes)]
    .map(noteToMidi)
  ...
  const nOct = Math.max(1, Math.round(octaves))
  let notes = []
  for (let o = 0; o < nOct; o++) for (const p of midi) notes.push(p + 12 * o)
  if (notes.length > MAX_ARP_NOTES) notes = notes.slice(0, MAX_ARP_NOTES)

  return orderArp(notes, style).map(midiToNote)
}
...
// Deterministic note-per-stop map for a route. The base mapping is two-axis
// geographic: latitude picks the scale degree and longitude picks the octave
// register (see geoToMidi) — pitch and rhythm come from the same stop object,
// so the rail is literally a melody.
//
// `opts` layers seeded variety on top (all deterministic per route/stop, so a
// reloaded song replays the same notes — see docs/gtfs-salt.md):
//   contour  'demand'     — preprocessed service/ridership demand → register
//                           (the default: busier stops sit higher)
//            'geographic' — latitude → degree (the original base mapping)
//            'randomWalk' — seeded ±step walk through scale degrees, pulled
//                           back toward the route's geographic center line
//            'arch'       — rises then falls along the stop sequence
//   variety  0..1 — 0 reproduces the pure contour map bit-for-bit (no jitter,
//            no per-route signature transposition); higher
//            widens the degree range toward two octaves, adds per-stop salted
//            jitter, a per-route signature transposition, and lets long
//            inter-stop gaps produce bigger melodic leaps
//   routeId  seeds the per-route/per-stop salts

export const PITCH_CONTOURS = ['demand', 'geographic', 'randomWalk', 'arch']
export const DEFAULT_PITCH_VARIETY = { contour: 'demand', variety: 0 }

// Per-stop gap factor: each stop's distance to the previous stop relative to the
// route's mean gap, clamped to [0.5, 2]. 1 = average spacing; >1 = long run
// before this stop. All 1s when the stops carry no usable `dist`.
function stopGapFactors(stops) {
  const n = stops.length
  const factors = new Array(n).fill(1)
  if (n < 2) return factors
  const gaps = []
  for (let i = 1; i < n; i++) {
    const d = (stops[i].dist ?? 0) - (stops[i - 1].dist ?? 0)
    gaps.push(d > 0 ? d : 0)
  }
  const mean = gaps.reduce((s, g) => s + g, 0) / gaps.length
  if (!(mean > 0)) return factors
  for (let i = 1; i < n; i++) {
    factors[i] = Math.max(0.5, Math.min(2, gaps[i - 1] / mean))
  }
  return factors
}

export function generatePitchMap(stops, rootMidi = 62, modeScale = MODES.dorian, octaveSpan = 3, opts = {}) {
  if (!stops?.length) return []
  const bounds = routeBounds(stops)
  // Keep this default in sync with DEFAULT_PITCH_VARIETY: callers spread a
  // possibly-absent per-track config into `opts`, so the fallback lives here too.
  const { contour = 'demand', variety = 0, routeId = '' } = opts
  const v = Math.max(0, Math.min(1, variety))

  // Legacy path — must stay bit-for-bit identical for saved songs (which
  // songState pins to 'geographic' when migrating anything older than v4).
  if (contour === 'geographic' && v === 0) {
    return stops.map(s => midiToNote(geoToMidi(s.lat, s.lon ?? s.lng, rootMidi, modeScale, octaveSpan, bounds)))
  }

  // Data contour: a stop's city-normalized `signals.demand` owns the full
  // register, so more-used stops are unambiguously higher. The preprocessor
  // prefers ridership and falls back to scheduled departures + interchange
  // centrality. Old route JSON has no signals; fall back to the existing
  // geographic contour instead of producing a flat or broken melody.
  if (contour === 'demand') {
    const hasDemand = stops.some(s => Number.isFinite(s?.signals?.demand))
    if (!hasDemand) return generatePitchMap(stops, rootMidi, modeScale, octaveSpan, { ...opts, contour: 'geographic' })
    const N = modeScale.length
    const maxDeg = Math.max(0, octaveSpan * N - 1)
    const gapFactors = stopGapFactors(stops)
    return stops.map((s, i) => {
      const lat = s.lat ?? bounds.latMin
      const geographicFallback = normSpan(lat, bounds.latMin, bounds.latMax, bounds.minSpan)
      const demand = Number.isFinite(s?.signals?.demand)
        ? Math.max(0, Math.min(1, s.signals.demand))
        : geographicFallback
      // Variety is deliberately bounded: the underlying demand ranking stays
      // legible while repeated stops avoid sounding mechanically identical.
      const jitter = (hashStopValue(routeId, s.id ?? i) - 0.5) * 4 * v * gapFactors[i]
      const degree = Math.max(0, Math.min(maxDeg, Math.round(demand * maxDeg + jitter)))
      return midiToNote(rootMidi + modeScale[degree % N] + Math.floor(degree / N) * 12)
    })
  }

  const N = modeScale.length
  // Degree range widens from one scale octave toward two as variety rises.
  const maxDeg = 2 * N - 1
  const span = (N - 1) + v * N
  // Per-route signature: a diatonic offset so different lines don't all sit on
  // the same degrees. Scaled by variety, seeded by route id.
  const sigOffset = Math.round(mulberry32(makeSalt(routeId, 'sig'))() * (N - 1) * v)
  const gapFactors = stopGapFactors(stops)
  const walkRng = mulberry32(makeSalt(routeId, 'walk'))
  let walkPos = span / 2

  return stops.map((s, i) => {
    const lat = s.lat ?? bounds.latMin
    const lng = s.lon ?? s.lng ?? bounds.lngMin
    const latT = normSpan(lat, bounds.latMin, bounds.latMax, bounds.minSpan)
    const lngT = normSpan(lng, bounds.lngMin, bounds.lngMax, bounds.minSpan)

    let baseDeg
    if (contour === 'randomWalk') {
      if (i > 0) {
        // ±1–2 degree step (bigger with variety and after a long gap), with a
        // pull back toward the route's geographic center line.
        const stepSize = (1 + Math.round(walkRng() * (1 + v))) * gapFactors[i]
        const dir = walkRng() < 0.5 ? -1 : 1
        const pull = (latT * span - walkPos) * 0.3
        walkPos += dir * stepSize + pull
        if (walkPos < 0) walkPos = -walkPos
        if (walkPos > span) walkPos = 2 * span - walkPos
      }
      baseDeg = walkPos
    } else if (contour === 'arch') {
      const seqT = normalizeStopSequence(i, stops.length)
      baseDeg = Math.sin(Math.PI * seqT) * span
    } else {
      baseDeg = latT * span
    }

    // Salted per-stop jitter (±2 degrees max), amplified after long gaps.
    const jitter = (hashStopValue(routeId, s.id ?? i) - 0.5) * 4 * v * gapFactors[i]

    const deg = Math.max(0, Math.min(maxDeg, Math.round(baseDeg + jitter) + sigOffset))
    const octave = Math.round(lngT * (octaveSpan - 1))
    const midi = rootMidi + modeScale[deg % N] + Math.floor(deg / N) * 12 + octave * 12
    return midiToNote(midi)
  })
}

// Per-stop trigger velocity (0.6–1.0), parallel to generatePitchMap's output.
// Flat 1.0 at variety 0 (Tone's default — old behavior). With variety, stops
// after long inter-stop gaps keep full velocity and tightly packed stops soften,
// so long runs read as accents — same gap normalization as the pitch map.
export function generateVelocityMap(stops, variety = 0) {
  if (!stops?.length) return []
  const v = Math.max(0, Math.min(1, variety))
  if (v === 0) return stops.map(() => 1)
  const gapFactors = stopGapFactors(stops)
  return gapFactors.map(g => Math.max(0.6, Math.min(1, 1 + (g - 1) * 0.4 * v)))
}
...
// Per-track note-grid resolution options (Tone.js-style rate strings, shared with the
// arpeggiator's ARP_RATES vocabulary above). Maps each rate to how many grid steps make
// up one 4/4 bar — straight subdivisions plus 8th/16th triplets.
export const GRID_RESOLUTION_STEPS_PER_BAR = {
  '4n': 4, '8n': 8, '8t': 12, '16n': 16, '16t': 24, '32n': 32,
}
export const DEFAULT_GRID_RESOLUTION = '16n'

// Map each stop's distance position to a grid cell (default: 16th-note cells).
// Collision rule: bump forward to the next free cell so no two stops share a cell.
// Stops that fall beyond the last cell are dropped.
// Returns an array of stop objects extended with:
//   cellIdx     – 0-based cell index (0 – totalCells-1)
//   originalIdx – original index in the input array (for pitchMap lookup)
//   bar         – 0-based bar number
//   beat        – 0-based beat within bar
//   sixteenth   – 0-based step within beat (name kept for the default 16th-note case;
//                 represents whatever subdivision `stepsPerBar` implies otherwise)
export function snapStopsToGrid(stops, totalDist, totalCells = GRID_TOTAL_CELLS, stepsPerBar = GRID_STEPS_PER_BAR) {
  ...
  for (let i = 0; i < stops.length; i++) {
    ...
    out.push({
      ...
      cellIdx:     cell,
      originalIdx: i,
      bar:         Math.floor(cell / stepsPerBar),
      beat:        Math.floor((cell % stepsPerBar) / stepsPerBeat),
      sixteenth:   cell % stepsPerBeat,
    })
  }
  return out
}

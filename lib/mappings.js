...
//   contour  'geographic' — latitude → degree (default, the base mapping)
//            'demand'     — preprocessed service/ridership demand → register
...
//   variety  0..1 — 0 reproduces the pure geographic map bit-for-bit; higher
...
export const PITCH_CONTOURS = ['geographic', 'demand', 'randomWalk', 'arch']
export const DEFAULT_PITCH_VARIETY = { contour: 'geographic', variety: 0 }
...
export function generatePitchMap(stops, rootMidi = 62, modeScale = MODES.dorian, octaveSpan = 3, opts = {}) {
  ...
  const { contour = 'geographic', variety = 0, routeId = '' } = opts
  const v = Math.max(0, Math.min(1, variety))

  // Legacy path — must stay bit-for-bit identical for saved songs.

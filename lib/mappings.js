...
export function buildArpSequence(rootNote, cfg = {}, scaleType = 'major') {
  const { octaves = 1, steps = 3, distance = 2, style = 'up' } = cfg
  const rootMidi = noteToMidi(rootNote)
  const iv = SCALES[scaleType] ?? SCALES.major
  const N = iv.length

  // Base chord tones: tone i sits at scale degree i*distance above the root.
  ...
  for (let i = 0; i < nSteps; i++) {
    const deg = i * dist
    base.push(rootMidi + iv[deg % N] + 12 * Math.floor(deg / N))
  }

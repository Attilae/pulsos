...
import {
  generatePitchMap, generateVelocityMap, shiftOctaveNote, shiftSemitones, noteToMidi, randomFromScale,
...
export function buildLoopMidiEvents(route, ctx) {
  ...
  const raw = gridStops.map(stop => {
    ...
    const shifted = shiftSemitones(tuned, ctx.trackSemitones?.[route.id] ?? 0)

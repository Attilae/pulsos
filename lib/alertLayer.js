...
//   lineType volumes → masterReverb → masterCompressor → masterLimiter → output
//
// `output` defaults to the Destination but the engine passes its own master gain,
// so two engines alive at once (the Song Chainer's A/B handoff) can be faded
// independently — the single shared Destination volume can't tell them apart.
export class AlertLayer {
  constructor(output = Tone.getDestination()) {
    ...
    // Chain: reverb → compressor → limiter → output
    this.reverb.chain(this.compressor, this.limiter, output)

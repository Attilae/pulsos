...
4. **Shared song service.** Extract the user-scoped preset database logic from `app/api/presets/*` into a server module called by both HTTP routes and MCP tools. The current AI plan validator lives in `lib/ai/composer.js` and imports browser/audio code, so extract a pure server-safe validator before accepting AI-generated plans through MCP. Likewise, extract any pure plan-to-snapshot operation needed for saving changes; do not import `MixerTab.jsx` or Tone.js on the server. Use version checks for updates so an MCP client cannot silently overwrite a newer browser save.
5. **Tools.** The first four are `list_cities`, `list_songs`, `get_song(songId)`, and `set_song_tempo(songId, bpm, expectedUpdatedAt)`. They establish ownership, read, and conflict-safe write behavior without changing snapshot topology. Extend Phase 2 with bounded route lookup, a pure plan validator, `preview_song_plan`, and `save_song_plan` before treating MCP as a full replacement for in-app composition. Return compact structured results and validation warnings. A preview is read-only; a save writes only the authenticated user's song and does not trigger playback or export. Mark tool annotations accurately, but enforce safety server-side because annotations are only hints to clients.

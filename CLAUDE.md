...
> **Note:** `AGENTS.md` at the repo root is **generated** from this file for Codex — only the
> header differs. After editing this file, run `npm run sync:agents` (`npm run sync:agents --
> --check` exits non-zero if the two have drifted). Never hand-edit `AGENTS.md`.
...
npm run build:skill  # zip skills/leid-composer → public/skills/leid-composer.zip (auto via prebuild/predev)
...
- **Composer skill**: `skills/leid-composer/` is a user-installable Agent Skill (`SKILL.md` +
  `references/recipes.md`/`troubleshooting.md`) that teaches a connected Claude the compose/edit
  workflow and genre recipes. It deliberately **copies no vocabulary** — enums and ranges come from
  `get_composer_guide` at call time — but its example plans do name instruments/FX/values, so
  `test/composer-skill.test.js` fails if any is dropped *or silently clamped* by `validatePlan`. A
  vocabulary change that breaks a recipe fails `npm test`; fix the recipe. `scripts/build_skill.js`
  zips it to `public/skills/leid-composer.zip` (gitignored, regenerated by `prebuild`/`predev`), which
  `McpSection` offers as a download.
- **Public guides**: `app/guides/mcp` (connecting a client, tool list, limits, disconnecting) and
  `app/guides/composer-skill` (what the skill is, install steps) — both `notFound()` unless
  `NEXT_PUBLIC_MCP_ENABLED`, and linked from `McpSection`. They render through `GuideShell`
  (`components/guides/`), which is `LegalShell` with its now-optional `navItems`/`meta`/`footerPrompt`
  overridden. The MCP URL shown is derived from `BETTER_AUTH_URL` the same way `lib/auth.js` does,
  at build time (the pages are static). Keep the tool table in sync when a tool is added.
